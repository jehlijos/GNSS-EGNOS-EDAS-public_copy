...
# GSV
# GST
# EGNOS GNSS RECIEVER, PORT SYMBOLIC LINK: downblue, RECIEVER =2 
...
reciever = 2  # ID by type of reciever (EGNOSxEDAS)
i = 0
i_gsa = 0
i_gsv = 0
egnos = 0
...
with serial.Serial("/dev/downblue", 38400, timeout=1) as ser:
    while now_time < end_time:
        ...
        if len(line) < 6:  # short line skiped 
           ...
           # print(line)
            ...
            if   gsv_write_time <=timestamp:
                ...
                if len(gsv.data) == 7:
                    ...
                    for element in gsv_tuple[1:]:
                        if str(element).isdigit():
                            gsv_tuple2 += (int(element),)
                        else: # converts everything else than int into int, except for datetime
                            gsv_tuple2 += (0,)
                    gsv_tuple = (gsv_tuple[0],) + gsv_tuple2
            ...
            except: print("no tracked_sats")#adding gsv sum of sats every sec
